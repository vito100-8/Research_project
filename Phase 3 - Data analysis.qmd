...
joined_dataset <- readRDS("data/joined_dataset.rds")
```
...
Even if the dataset comprises energy-related facilities, those categorized under other NAICS sectors, such as sector 62 or 72 likely involve other activities as part of or adjacent to the energy infrastructure.
...
dataset_summary <- joined_dataset |>
  ...
  summarise(
    ...
    "Periodicity" = "Annual",
    "Extremum levels \n of CO2 emissions " = paste(range(`Total CO2 Emissions (non_biogenic)`), collapse = " - "),
    ...
    "Total number of missing \n values across the sheet" = sum(sapply(joined_dataset, function(x) sum(is.na(x)))),
  )
...
NAICS2_level_summary <- joined_dataset |>
  ...
  summarise(
   ...
   #Contribution of one sector in total emissions divided by the emissions of all sectors 
    "Contribution (%) of California \n to the total CO2 emissions of the sector" = 
...
dataset_summary |>
  kable("latex", caption = "Global database information",format.args = list(decimal.mark = ".", digits = 2)) |>
  kable_styling( latex_options = c("striped", "hover", "condensed", "scale_down", "line", "hold_position")) 


NAICS2_level_summary |>
  kable("latex", caption = "Per NAICS 2 database information",format.args = list(decimal.mark = ".", digits = 2)) |>
  kable_styling(latex_options = c("striped", "hover", "condensed", "scale_down", "line", "hold_position")) 
...
joined_dataset |>
  ...
  labs(title = "Evolution of Total Employment by Sector",
       x = "Year") +
  theme(legend.position.inside = c(1.55, 0.5)) +
...
joined_dataset |>
  ...
  labs(title = "Evolution of Average Monthly Wage \n by Sector",
    x = "Year") +
  theme(legend.position.inside = c(1.55, 0.5)) +
  theme_minimal()
...
The following set of graphs show, respectively, the evolution of `avg_EarnS` and CO2 Emissions and the evolution of `total_Emp` and CO2 Emissions, both at a sectoral level and by State.
...
joined_dataset |>
  ...
  labs(
    title = "California's CO2 Emissions & \n Average Wage",
    x = "Year"
  ) +
  theme(legend.position.inside = c(1.55, 0.5)) +
  theme_minimal()
...
joined_dataset |>
  ...
  labs(title = "Texas' CO2 Emissions & Average Wage",
    x = "Year"
  ) +
  theme(legend.position.inside = c(1.55, 0.5)) +
  theme_minimal()
...
joined_dataset |>
  ...
  labs(title = "Texas' CO2 Emissions & \n Total Employment",
    x = "Year"
  ) +
  theme(legend.position.inside = c(1.55, 0.5)) +
  theme_minimal()
...
# Calculating Q (Total Employment), S (Activity Share), and I (Intensity) and adding it into a dataset
...
#we defin the years we are interested in 
base_year <- 2013
target_year <- 2023
...
# Decomposition using the  framework explained
lmdi_results <- joined_dataset_2023 |>
  ## performing a join to have both 2013 and 2023 values in one dataset to be able to compute the difference/evolution of variables
  left_join(joined_dataset_2013, by = "industry", suffix = c("", "_base")) |>
  ## we filter out the sector 42 since its data don't cover the period as visible in the previous graphs
  ...
  mutate(
    ...
    ## variation of each driver (industry level)
  ...
  group_by(`NAICS2  (sectors)`) |>
  summarize(
    ##computing the contribution of each driver (sector level)
...
#Adding contribution in percentage of CO2 total variations (allowing for negative contributions and  values>100%)
...
#Creation of a table for the percentage contribution analysis
contribution_summary_table_percent <- contribution_summary_table

 contribution_summary_table_percent |>
  mutate(
  "Activity contribution (in perc)" = A_Percentage,
    "Structure contribution (in perc)" = S_Percentage,
    "Intensity contribution (in perc)" = I_Percentage,
  .keep = "none"
  ) |>
  kable(
    "latex",
    caption = "Contribution of each factor (in percentage)", 
    format.args = list(decimal.mark = ".", digits = 3)
    ) |>
  kable_styling(
    latex_options = c("striped", "hover", "condensed", "scale_down", "line", "hold_position")
    ) 
 
# Table summarizing contributions in values
contribution_summary_table_values  <- contribution_summary_table
contribution_summary_table_values |>
  mutate(
  ...
  kable(
    "latex",
    caption = "Contribution of each factor (values)",
    format.args = list(decimal.mark = ".", digits = 3)
    ) |>
  kable_styling(
    latex_options = c("striped", "hover", "condensed", "scale_down", "line", "hold_position")
...
First, we observe the difference in variations among sectors which correlates with the magnitude of each sector and the CO2 emissions associated (Table 4). A huge sector in terms of emission (e.g. 21) provokes more CO2 variations across time but encompasses, as we’ve seen, more firms. As we are more interested in relative contribution, those values only provide information on the magnitude of the variation without giving a clear view of their drivers. Nonetheless we observe the trend for CO2 emissions of each sector and still find the huge positive variations for sectors such as 21 and 32 similar to the hikes described in the graphical representations.

Then, if we analyse the contribution in percentage (Table 3) we can observe the relative importance of a factor on CO2 emissions in a specific sector between 2013 and 2023. The sum of the three drivers sums up to 100% but there is, at this point, no normalization on the values; thus, we can observe contributions of more than 100% if the variations of a factor is higher than the ones related to CO2 emissions. An interesting finding is the extremely high values for some sectors such as 22 (Utilities) which encompass for example electric power generation and natural gas distribution. For this case we observe that the Intensity factor has a great positive contribution in the variation (1141%) suggesting that the increase in carbon intensity of this sector was large but covered by the decreasing activity and the shrinking relative part of the industry in the total sector.

Overall, the Intensity factor has a positive coefficient, underlining the fact that increased carbon-intense activities per industry produces more C02 emissions. The other two contributions are less straightforward since Activity is theoretically a positive driver of emissions since an increased workforce size in the sector should translate into more production and, thus, more emissions. Hence, its negative coefficients in 8 sectors, and of great magnitude in sectors 54, 48 and 22, seems surprising. Possible explanation lies in investment on infrastructures between 2013 and 2023. For example, the Texan power plants associated with the Utilities sector may have increased their technology and their energy mix resulting in lower emissions even though their output is increasing. Another one is the close-to-zero emission values of small sectors that provoke a more imprecise contribution analysis (as an outlier). In this case the sector 54 that represents 0.0008% of total emissions seems to embody this aspect. Finally, the structure appears to also have 8 negative coefficients, meaning that the increase of a firm in its sector would decrease its emission. This result is quite surprising because we expect the opposite, as in sector 21 with increased relative share of the industry linked to higher emissions. Investment in cleaner plants and adoption of greener technologies could be a solution, since an increased share often translates into more budget available for innovation.
...
# Graphical representation that uses normalized data (contribution between 0-100%)
normalized_data <- contribution_summary_table |>
  mutate(
    ##taking absolute value to have relative normalized contribution
  ...
  ##reshaping to have an appropriate graphical format with the contributive variables as "Factor"
  pivot_longer(
...
# Plotting the values with bar filled (0-100%) to see the relative contribution of each factor for each sector (absolute values)
normalized_data |> 
...
For a clearer analysis, we normalize the contribution to see the relative contribution of each driver (in percentage) on emissions. We observe that the intensity factor plays a greater role overall, underlining the fact that carbon efficiency (here emissions divided by the workforce size of the industry) is crucial for energy-producing firms.
...
# Summarizing average contributions across all sectors between the years
relative_contrib_between_var <- normalized_data |>
  group_by(Factor) |> 
  ## It is important to note that the choice made is to take the mean of all normalized contributions meaning that if a variable explains a lot of a small industry's variations in emissions its relative importance will be higher than a variable explaining only a bit of a huge industry with enormous variations.
  summarize(Average_Percentage_Contribution = mean(Normalized_Percentage_Contribution, na.rm = TRUE))

# Renaming factors to be clearer
...
# Horizontal bar chart to see the relative importance of factors
...
The latter observation is confirmed by this chart since it shows that the relative importance of Intensity is higher than the importance of the other factors combined. Particularly we observe that the share of the industry is the least important factor, meaning that the sector share of an industry is not the main driver of carbon emissions.

As we’ve seen sectors 21 and 22 represent 98% of total emissions, we will perform a LMDI decomposition on these sectors and separate the State to better analyze them according to their location. The first chart will be in values to display the difference of magnitude between them and the second in relative contribution.
...
# Creating a subset that includes our wanted sectors
joined_subset_preparation <- joined_dataset |> 
   ## Keep only sectors 21 and 22
  filter(`NAICS2  (sectors)` %in% c(21, 22)) |> 
 
  group_by(year, `NAICS2  (sectors)`) |>
  ## Compute Total Employment per sector
  mutate(Total_Emp_sector_2 = sum(total_Emp, na.rm = TRUE)) |> 
  ungroup() |>
...
# Creation of the subset of our dataset for the years needed 
...
# Perform LMDI decomposition with the same framework and steps but  using the prepared subset 
...
# Preparing data for contributions (in values) visualization
...
# Plotting absolute contributions
...
We first notice the difference of contribution depending on the State : it emphasizes the importance of Texas in emissions, especially compared to California. It then appears that the majority of the emission’s increases in Texas is linked to Intensity. It reveals that the hike we previously mentioned is probably due to a worse energy mix and technology of production, the fracking which increased between the two periods is an example of such worsening.

```{r}
#| echo: false
#| message: false
#| warning: false
# Normalizing contributions for percentage analysis as in the general LMDI
...
# Plotting relative contributions
...
The relative contribution also displays the greater role of Intensity for each sector, except for the 21 sectors in California where the structure has a higher contribution. Perhaps, it comes from the fact that California implemented environmental measures, for example the Cap-and-Trade studied in the next part, in the time period, thus resulting in significant share changes between the industries depending on their reactions and energetic mix. Activity maintains a contribution around 10% for each sector denoting a relative quasi-stability of its effect on those industries (although the opposite reaction of each industry's emissions to the factor).
...
$$
log(CO_2 \space emissions) = \beta_0 + \beta_1log(Earnings) + \beta_2log(Employment) + \epsilon
$$
...
dataset_for_ols <- joined_dataset |> 
  ...
  mutate(`ln Total CO2 Emissions` = log(`Total CO2 Emissions (non_biogenic)`), `ln Employment` = log(total_Emp), `ln Earnings` = log(avg_EarnS))
...
base_ols_model <- lm(`ln Total CO2 Emissions` ~  `ln Earnings` + `ln Employment`, data = dataset_for_ols)
...
tidy_model <- tidy(base_ols_model)

xtable_table <- xtable(tidy_model)
xtable::print.xtable(
  xtable_table,
...
$$
log(CO_2 \space emissions) = \beta_0 + \beta_1log(Earnings) + \beta_2log(Employment) + \beta_3Policy + \epsilon
$$
...
dataset_for_ols <-
  dataset_for_ols |>
  mutate(Policy = if_else(State == "CA" & year > 2015, 1, 0))

extend_ols_model <- lm(`ln Total CO2 Emissions` ~  `ln Earnings` + `ln Employment` + Policy, data = dataset_for_ols)
...
xtable_table <- xtable(extend_ols_model)
xtable::print.xtable(
  xtable_table,
  include.rownames = FALSE,
  comment = FALSE
...
As in the previous results, we still have a positive and highly significant correlation between average monthly wage and CO2 emissions per industry : A 1% increase in average wages is associated with approximately a 3.16% increase in CO2 emissions, keeping all other variables constant. On the contrary ln emp is still not statistically significant (p-value = 0.134) and is still positive. Finally, the dummy `Policy` proves to be highly significant (p-value \< 0.01) and positive. We could infer that firms in California after the introduction of the cap-and-trade program in 2015 emit approximately 90% less CO2 than firms in Texas during the same period, controlling for wages and employment. This result confirms our main hypothesis : The cap-and-trade program has a significant effect on the emissions of industries in our dataset.
...
# running the bptest and extracting relevant values
bp_result <- bptest(extend_ols_model)

# Extraction of  test statistics and p-value
bp_values <- data.frame(
...
# creation of  a LaTeX table

xtable_table <- xtable(bp_values)
xtable::print.xtable(
  xtable_table,
  include.rownames = FALSE,
  comment = FALSE
...
$$
log(CO_2 \space emissions) = \beta_0 + \beta_1log(Earnings) + \beta_2log(Employment) + \beta_3Policy + u_t + \epsilon 
$$

where `u` corresponds to sectoral (NAICS 2) fixed effect i.e.controlling for unobserved heterogeneity across sectors. Additionally, we will implement robust standard errors to correct further our heteroscedasticity and normality issues.
...
fe_ols_model <- felm(log(`Total CO2 Emissions (non_biogenic)`) ~  `ln Earnings` + `ln Employment` + Policy | `NAICS2  (sectors)`, data = dataset_for_ols)

## Summary of fixed-effect model


xtable_table <- xtable(fe_ols_model)
xtable::print.xtable(
  xtable_table,
  include.rownames = FALSE,
  comment = FALSE
...
In this work we studied the CO2 emissions of energy-producing firms in California and Texas and found that, according to the EIA's terminology and its data, Texas' firms emit, on average, a lot more than their Californian counterparts. This is particularly true for the Mining, Quarrying, and Oil and Gas Extraction (21) and Utilities (22) sectors that amount for nearly all emissions across our dataset. We observed that wages were correlated positively with the CO2 emissions while the pattern with total employment was more opaque. The LMDI decomposition method was used to evaluate the contributions of Quantity, Structural and Intensity drivers on the emissions by using total employment as proxy for industry size (and inputs utilization). We determined that carbon-intensity was the largest contribution on average while pure quantity effect was the smallest, revealing potential weakness in our choice of proxy. Finally, we developed a regression model to assess the effect of an environmental policies and confirmed the small significance of total employment. The results led us to conclude that is had a substantial effect on the reduction of C02 emissions. Moreover, earnings were also highlighted as a significant drivers of carbon emissions across our models. The work has several limitations. The main ones that occured to us are the data coverage, with several missing values in both datasheets that were not treated but filtered out, which may have induced bias in our results; the lack of control with traditional variables to avoid omitted variable bias, for example we did not include data about energy consumption (used in the vast majority of research papers).
