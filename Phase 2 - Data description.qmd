...
clean_em_data <- emissions_data |> 
  filter(State == "CA" | State == "TX") |>  # We only keep Californian and Texas firms
  mutate(
...
## Cleaning for the NA values of Emissions
joined_dataset <- joined_dataset |> filter(is.na(`Total CO2 Emissions (non_biogenic)`) == FALSE)

```

We now have our join database combined of worforce and emissions previous datasets
