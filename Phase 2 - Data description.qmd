...
https://lehd.ces.census.gov/data/qwi/latest_release/al/qwi_tx_se_f_gs_n4_oslp_u.csv.gz
...
workforce_summary_CA <- workforce_data |>
  summarise(
...
clean_em_data <- emissions_data |> 
  filter(State == "CA" | State == "TX") |>  # We only keep Californian firms
  mutate(
    `NAICS 4 (subsectors)` = substr(`Primary NAICS Code`, 1, 4), #agregation at NAICS 4 level
    # To avoid issues with the NA numbers during the sum after with parse number (convert from character to numerical)
  ...
  # Group by NAICS subsectors, reporting year and unit type
  group_by(`NAICS 4 (subsectors)`, `Reporting Year`) |> 
  summarise(
...
```{r}
#Merging the two using wf data as master, so we can keep all observations from the latter dataset
joined_dataset <- clean_wf_data  |>
  left_join(clean_em_data)
```
...
```{r}
#Graphical representation by sectors that are energy-producing
joined_dataset <- joined_dataset |> mutate(
    `NAICS2  (sectors)` = substr(industry, 1, 2 ), .keep = "all")

joined_dataset |> 
  filter(!is.na(`Total CO2 Emissions (non_biogenic)`)) |>
  ggplot(aes( x =  `Total CO2 Emissions (non_biogenic)`, color = `NAICS2  (sectors)`, fill = `NAICS2  (sectors)`) ) +
  geom_density( na.rm = TRUE) + 
  scale_x_log10() +
  facet_wrap(vars(`NAICS2  (sectors)`))
