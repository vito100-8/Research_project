...
and $\Delta E_I = \sum_i L(E^T_i,E^0_i)*ln(I_i^T/I_i^0)$ with $L (x,y)= (x-y) /(ln (x) - ln (y))$ or $L(x,y) = x$ for $x = y$ defined as the logarithm mean.
...
$$
log(CO_2 \space emissions) = \beta_0 + log(Earnings)\beta_1 + log(Employment)\beta_2 + Policy\beta_3 + \epsilon
$$
...
$$
log(CO_2 \space emissions) = \beta_0 + log(Earnings)\beta_1 + log(Employment)\beta_2 + Policy\beta_3 \space\|\space NAICS 2 \space sectors + \epsilon
$$
