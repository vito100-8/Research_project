...
emissions_summary <- emissions_data |>
  summarise(
    ...
    "Missing values in CO2 emissions" = sum(is.na(`Unit CO2 emissions (non-biogenic)`))
  )
...
workforce_ca_summary <- workforce_ca_data |>
  summarise(
    ...
    "Missing values of Total Employment" = sum(is.na(EmpTotal)),
    "Missing values of Monthly earnings" = sum(is.na(EarnS))
  )
...
### Joining the workforce datasets and summarizing it
...
For the CO2 emissions dataset it appears that the missing values appear in `FRS id`, `Unit Type` and `Unit Maximum Rated Heat Input Capacity (mmBTU/hr)` and none in our variables of interest  nor in the variables that will allow us to merge the two datasets. Those missing values come from information not available probably because the firms did not transmitted it to the EPA.

For the workforce datasets we notice a huge amount of missing values accross the datasets, for our variables of interest we notice respectively 6942 and 26513 missing values for `EmpTotal`and `EarnS`. 
The Quarterly workforce Indicators (QWI) uses data from "administrative records data collected by a large
number of states for both jobs and firms, and enhanced with information integrated from other data
sources at the Census Bureau" ([See source](https://lehd.ces.census.gov/doc/technical_paper/tp-2006-01.pdf)). Hence these indicators comes from sources used by the LEHD structure and, except for some variables such as `sex`, the missing values are not adressed nor treated; they represent non-available information or non-disclosed to the public. It appears that the more recent the data, the less there are missing values (considering the increasing data production over the recent years).
...
`Primary NAICS Code` : this variable identifies each firm and will allow us to merge the two datasets by agregating each firm into a NAICS code 4 (at the level of industries). The North American Industry Classification System (NAICS) is a standardized system used by the U.S., Canada, and Mexico to classify business establishments based on their economic activities. 
...
After we cleaned up the datasets, we first merged the `clean_wf_tx_data` and the `clean_wf_ca_data` datasets together to be stacked one under the other. This way, the observations from the two datasets can only be differenciated by the `geography column`, which contains the geography code corresponding to the two chosen States.
...
Finally, we did a `inner_join()` merge of the two final datasets. As the workforce datasets contained observations for all industry sectors, while the `emissions_data` dataset only contained observations for energy producing sectors, we did a final filtering to drop the missing values corresponding to non-energy producing sectors (according to the definition in the Research Question part). 
...
```{r}
#| message: false
# Merging emissions and workforce datasets using workforce data as master

#Preparing the merge by agregating the workforce dataset : we want to have a single value of our key variables to be linked to variables in the emission dataset
#To do that, we take the mean of our variables (except for employment where the quantity aspect need to be summed) and agregate them at all education and sex levels. 
#Thus we group by industry and year (the level of detail wanted for our later join)

join_wf_data <- join_wf_data |>
  group_by(industry, year, State) %>%
  summarize(
    avg_EarnS = mean(avg_EarnS, na.rm = TRUE),
    total_Emp = sum(avg_Emp, na.rm = TRUE),
    total_FrmJbC = sum(avg_FrmJbC, na.rm = TRUE),
    .groups = "drop"
  )
#inner join with id variables detected being industry and year
joined_dataset <- join_wf_data |> inner_join(clean_em_data)
```
...
We constructed two charts :  
  ...
  - the first chart describes the evolution over time of `CO2 emissions (non biogenic)` for each `State`. We added a `NAICS2  (sectors)` variable to compute the absolute contribution of each sector(NAICS 2) for each state in a given year.  Additionnal information on the NAICS structure can be found at the following link : [Description of NAICS 2 sectors](https://www.census.gov/naics/?58967?yearbck=2022)
...
We will be investigating the relationship between workforce characteristics (employment count, wage...) and CO2 emissions in energy-producing firms in California and Texas from 2013 to 2023. We distinguish three types of energy-producing firms according to the data source : 

- Direct emitters, which correspond to facilities that combust fuels or otherwise put green house gases into the atmosphere directly from their facility (ex. power plants).
...
Other key variable is Net employment change (`total_FrmJbC`) which will be used as a proxy for contraction or expansion of firms in an industry. The hypothesis is that industries with quick expansions may emit more due to production scale (and inverse effect for industries shrinking).
...
```{r}
# Saving the final dataset for the next phase
saveRDS(joined_dataset, file = "data/joined_dataset.rds")
```
