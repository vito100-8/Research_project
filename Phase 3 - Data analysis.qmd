...
joined_dataset |>
  ...
  summarise(
    contrib_co2 = 100 * sum(`Total CO2 Emissions (non_biogenic)`) / sum(joined_dataset$`Total CO2 Emissions (non_biogenic)`),
    avg_wage_sector = mean(avg_EarnS, na.rm = TRUE)) |>
  filter(`NAICS2  (sectors)` == "22") |>
  ggplot(aes(x = year)) + 
  geom_col(aes(y = contrib_co2, fill = "Sector CO2 Contribution")) +
  geom_point(aes(y = avg_wage_sector / 2100, color = "Average Monthly Wage")) +
  scale_y_continuous(
    name = "Contribution (%)",
    limits = c(0, 5),
    sec.axis = sec_axis(~ . * 2100, name = "Average Monthly Wage")) +
  scale_color_manual(
    values = c("Sector CO2 Contribution" = "red", "Average Monthly Wage" = "blue")) +
  facet_wrap(vars(State)) +
  labs(
    title = "Sector Contribution to CO2 Emissions and Average Monthly Wage",
    x = "Year"
  ) +
  theme_minimal() +
  theme(
