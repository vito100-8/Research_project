...
NAICS2_level_summary |>
  ...
  kable_styling( latex_options = c("striped", "hover", "condensed", "scale_down", "line")) 
...
$\leftrightarrow  E = \sum_iQ(Q_i/Q)(E_i/Q_i)$ (by multiplicating and dividing by $Q_i$ and $Q$)  thus we obtain $E = ∑_i(Q_iS_iI_i)$
...
$\Delta E_T = \Delta E_Q + \Delta E_S+\Delta E_I$ with
$\Delta E_Q = \sum_i L(E^T_i,E^0_i)*ln(Q_i^T/Q_i^0)$ ;
...
contribution_summary_table_values |>
  ...
  kable_styling(
    latex_options = c("striped", "hover", "condensed", "scale_down", "line")
    )

#Creation of a second table for the percentage contribution analysis
contribution_summary_table_percent <- contribution_summary_table
 ...
 contribution_summary_table_percent |>
  ...
  kable(
    "latex",
    caption = "Contribution of each factor (%)", 
    format.args = list(decimal.mark = ".", digits = 3)
    ) |>
  kable_styling(
...
# ploting the values with bar filled (0-100%) to see the relative contribution of each factor for each sector (absolute values)
normalized_data |> 
...
# Ploting absolute contributions
...
We firstly notice the difference of contribution depending on the State : it emphasizes the importance of Texas in emissions, especially compared to California.
It then  appears that the majority of the emission’s increases in Texas is linked to Intensity. It reveals that the hike we previously mentioned is probably due to a worse energy mix and technology of production, the fracking which increased between the two periods is an example of such worsening.
...
# Ploting relative contributions
