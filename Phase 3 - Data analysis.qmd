...
dataset_summary |>
  kable("latex", caption = "Global database information") |>
  kable_styling(full_width = T, latex_options = c("striped", "hover", "condensed"))


NAICS2_level_summary |>
  kable("latex", caption = "Per NAICS 2 database information") |>
  kable_styling(full_width = T, latex_options = c("striped", "hover", "condensed"))
```
...
contribution_summary_table |>
  ...
  kable("latex", caption = "Contribution of each factor") |>
  kable_styling(full_width = T, latex_options = c("striped", "hover", "condensed"))
...
$$
log(CO2 emissions) = \beta_0 + log(Average Monthly Wage)\beta_1 + log(Total Employment)\beta_2 + \epsilon
...
$$
log(CO2 emissions) = \beta_0 + log(Average Monthly Wage)\beta_1 + log(Total Employment)\beta_2 + Policy\beta_3 + \epsilon
$$
...
log(CO2 emissions) = log(Average Monthly Wage) + log(Total Employment) + Environment | NAICS 2 sectors
...
Applying a fixed-effect OLS with robust standard errors, using the `fixest` package, we now have the following final result : 
