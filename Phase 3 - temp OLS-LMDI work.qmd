...
# Fit an OLS model
test_data_21 <- joined_dataset |> 
  mutate(`NAICS2  (sectors)` = substr(industry, 1, 2 ), .keep = "all")|>
  filter(`NAICS2  (sectors)` == 21) |> rename ( `Total CO2 Emissions (non_biogenic) test` =  `Total CO2 Emissions (non_biogenic)`  )
...
ols_model <- lm(`ln Total CO2 Emissions test` ~  ln_earn + `ln emp`, data = test_data_21)

# View the summary of the model
summary(ols_model)

# Plot residuals to check model assumptions (optional)
par(mfrow = c(2, 2))
plot(ols_model)

```
```{r}
# Install the lfe package if not already installed
...
model <- felm(`ln Total CO2 Emissions test` ~  ln_earn + `ln emp` | year + State, data = test_data_21)
